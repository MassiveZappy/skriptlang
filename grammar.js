module.exports = grammar({
  ...
  rules: {
    ...
    keyword_tinyint: (_) =>
      choice(make_keyword("tinyint"), make_keyword("int1")),
    keyword_smallint: (_) =>
      choice(make_keyword("smallint"), make_keyword("int2")),
    keyword_mediumint: (_) =>
      choice(make_keyword("mediumint"), make_keyword("int3")),
    keyword_int: (_) =>
      choice(
        make_keyword("int"),
    ...
    _delete_statement: ($) =>
      seq($.delete, alias($._delete_from, $.from), optional($.returning)),
    ...
    function_language: ($) =>
      seq(
        ...
        // regard to the defined language to match either skript, plskript or
        // plpgskript. Currently the function_body_statement support only skript.  And
    ...
    _operator_class: ($) =>
      seq(
        ...
        optional(
          field(
            "opclass_parameters",
            wrapped_in_parenthesis(comma_list($.term)),
          ),
        ),
      ),
    ...
    index_fields: ($) =>
      wrapped_in_parenthesis(comma_list(alias($._index_field, $.field))),
    ...
    _create_trigger_event: ($) =>
      choice(
        ...
        seq(
          $.keyword_update,
          optional(seq($.keyword_of, comma_list($.identifier, true))),
        ),
        $.keyword_delete,
        $.keyword_truncate,
      ),
    ...
    // TODO: optional `keyword_add` is necessary to allow for chained alter statements in t-skript
    ...
    column_position: ($) =>
      choice(
        $.keyword_first,
        seq($.keyword_after, field("col_name", $.identifier)),
      ),
        ...
        choice(
          ...
          seq(
            ...
            choice(
              ...
              seq(
                $.keyword_reset,
                choice($.keyword_all, field("option", $.identifier)),
              ),
            ),
          ),
        ),
    ...
    set_schema: ($) =>
      seq($.keyword_set, $.keyword_schema, field("schema", $.identifier)),
    ...
    insert: ($) =>
      seq(
        ...
        optional(
          choice(
            $.keyword_into,
            $.keyword_overwrite, // Spark skript
          ),
        ),
        $.object_reference,
        optional($.table_partition), // Spark skript
    ...
    _compute_stats: ($) =>
      choice(
        ...
        seq(
          ...
          $.keyword_stats,
          $.object_reference,
          optional(choice(paren_list(repeat1($.field)), $._partition_spec)),
        ),
      ),
    ...
    update: ($) =>
      seq(
        $.keyword_update,
        optional($.keyword_only),
        choice($._myskript_update_statement, $._postgres_update_statement),
      ),

    _myskript_update_statement: ($) =>
      prec(
        0,
        seq(
    ...
    table_sort: ($) =>
      seq($.keyword_sort, $.keyword_by, paren_list($.identifier, true)),

    table_partition: ($) =>
      seq(
        choice(
          // Postgres/Myskript style
          seq(
            $.keyword_partition,
            $.keyword_by,
            choice($.keyword_range, $.keyword_hash),
          ),
          // Hive style
        ...
        choice(
          ...
          paren_list($._key_value_pair, true), // Spark skript
        ),
      ),
    ...
    table_option: ($) =>
      choice(
        ...
        seq(
          field(
            "name",
            choice($.keyword_engine, $.identifier, $._literal_string),
          ),
          "=",
    ...
    filter_expression: ($) =>
      seq($.keyword_filter, wrapped_in_parenthesis($.where)),
    ...
    partition_by: ($) =>
      seq($.keyword_partition, $.keyword_by, comma_list($._expression, true)),
    ...
    window_clause: ($) =>
      seq($.keyword_window, $.identifier, $.keyword_as, $.window_specification),
    ...
    values: ($) =>
      seq($.keyword_values, $.list, optional(repeat(seq(",", $.list)))),

    index_hint: ($) =>
      seq(
        choice($.keyword_force, $.keyword_use, $.keyword_ignore),
        $.keyword_index,
        optional(seq($.keyword_for, $.keyword_join)),
        wrapped_in_parenthesis(field("index_name", $.identifier)),
      ),
    ...
    _tskript_parameter: ($) => seq("@", $._identifier),
...
function unsigned_type($, type) {
  return choice(
    ...
    seq(type, optional($.keyword_unsigned), optional($.keyword_zerofill)),
  );
}
      ...
      seq(
        ...
        wrapped_in_parenthesis(
          seq(
            ...
            ...params.map((p) =>
              seq(",", field(p, alias($._natural_number, $.literal))),
            ),
          ),
        ),
      ),
